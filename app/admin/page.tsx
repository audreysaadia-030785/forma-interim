...
export default async function AdminDashboardPage({
  ...
  if (client !== "all") requests = requests.filter((r) => r.clientId === client);

  const pending = DEMO_REQUESTS.filter((r) => r.status === "pending").length;
  const proposed = DEMO_REQUESTS.filter((r) => r.status === "proposed").length;
  const validated = DEMO_REQUESTS.filter((r) => r.status === "validated").length;
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-8 sm:py-12">
      ...
      <section className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10 animate-fade-up">
        ...
        />
        <KPI
          label="Validations attendues"
          value={proposed}
          color="primary"
          href="/admin?status=proposed"
        />
        <KPI
          label="Missions validées"
          value={validated}
          color="success"
          href="/admin?status=validated"
        />
        <KPI
          label="Clients actifs"
          value={DEMO_CLIENTS.filter((c) => c.active).length}
          color="neutral"
          href="/admin/clients"
        />
      </section>
      ...
      {requests.length === 0 ? (
        <div className="rounded-[var(--radius-card)] border border-dashed border-neutral-300 bg-white p-12 text-center">
          <p className="text-neutral-500">Aucune demande dans cette catégorie.</p>
        </div>
      ) : (
...
function KPI({
  ...
  const content = (
    <div className="relative overflow-hidden rounded-[var(--radius-card)] bg-white p-5 ring-1 ring-neutral-200 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
      ...
      >
        {pulse ? <span className="relative mr-1 flex h-1.5 w-1.5">
